...
const Window = (props: WindowProps) => {
  ...
  let height = props.max ? winHeight : state.height;
  const disableMax = props.aspectRatio !== undefined;

  if(winHeight < 700) height -= 100;
  ...
  return (
    <Rnd
      ...
      className={`overflow-hidden cursor-default ${round} ${border} shadow-lg shadow-black/30 ${minimized}`}
      id={`window-${props.id}`}
    >
      <div
        className="window-bar relative h-6 text-center bg-[#8b8b8b]"
