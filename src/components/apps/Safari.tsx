      ...
      <div className={`mt-3 grid grid-flow-row ${grid}`}>
        {section.sites.map((site: SiteData) => (
          <div key={`safari-nav-${site.id}`} className="h-28 flex flex-col">
            <div className="size-16 mx-auto rounded-md overflow-hidden bg-white cursor-pointer">
              {site.img ? (
                <img
                  src={site.img}
                  alt={site.title}
...
const NavPage = ({ width, setGoURL }: NavProps) => {
  ...
  return (
    <div
      className="w-full safari-content bg-center bg-cover text-c-black"
      style={{
...
const NoInternetPage = () => {
  ...
  return (
    <div
      className="w-full safari-content bg-blue-50 bg-center bg-cover"
      style={{
        backgroundImage: `url(${dark ? wallpapers.night : wallpapers.day})`
      }}
    >
      ...
      {wifi ? (
        state.goURL === "" ? (
          ...
          <iframe
            ...
            src={state.goURL}
            className="safari-content w-full bg-white cursor-default"
          />
        )
      ) : (
        <NoInternetPage />
